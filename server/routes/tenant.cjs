...
function logAction(req, action, entityType, entityId, entityName, details) {
  // Fire-and-forget; ошибки логируем, но не возвращаем клиенту.
  queryInSchema(
...
router.post('/client-records', async (req, res, next) => {
  try {
    const {
...
router.put('/client-records/:id', async (req, res, next) => {
  try {
    const id = badId(req.params.id);
    if (!id) return res.status(400).json({ error: 'invalid_id' });
...
router.delete('/client-records/:id', async (req, res, next) => {
  try {
    const id = badId(req.params.id);
    if (!id) return res.status(400).json({ error: 'invalid_id' });
...
router.post('/entity-tags', async (req, res, next) => {
  try {
...
router.delete('/entity-tags/:id', async (req, res, next) => {
  try {
    const id = badId(req.params.id);
    if (!id) return res.status(400).json({ error: 'invalid_id' });
...
router.post('/data/:key', async (req, res, next) => {
  try {
